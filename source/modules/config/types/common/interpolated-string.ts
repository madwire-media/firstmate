...
import { ParamName } from './config-names';
...
export class InterpolatedString<C extends string = string> {
    public static compile<T extends t.Type<string> = t.Mixed>(
            ...
            if (match[1].length > 0) {
                ...
                if (isLeft(expressionResult)) {
                    return Result.Err(new ValidationError(
                        `Invalid expression: '${rawExpression}'`,
        ...
        strings.push(rawString.slice(lastIndex));
    ...
    public interpolateRaw(context: ExpressionContext): Result<string, InterpolationError> {
        let output = '';
    ...
    public interpolate(context: ExpressionContext): Result<C, InterpolationError> {
        const output = this.interpolateRaw(context).try();
...
export const interpolated = <C extends t.Mixed>(
    ...
    (inputString, context) => {
        if (inputString instanceof InterpolatedString) {
            return t.success(new InterpolatedString(
                inputString.strings,
                inputString.expressions,
                codec,
            ));
        }
